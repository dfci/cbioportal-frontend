...
> {
    ...
    renderCancerStudy = (study: CancerStudy, arrayIndex: number) => {
        ...
        const overlappingStudies = this.store
            .findOverlappingStudiesForStudy(study.studyId)
            .map(overlappingStudy => {
                const study = this.store.selectableSelectedStudies.find(
                    s => s.studyId == overlappingStudy
                );
